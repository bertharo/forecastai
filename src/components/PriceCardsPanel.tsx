...
/** Price cards panel — lives under Sources (connectors). */
export async function PriceCardsPanel() {
  await assertDb();
  ...
  return (
    <div id="price-cards" className="space-y-4">
      <div>
        <h2 className="text-[15px] font-bold">Price cards</h2>
        <p className="mt-1 text-[13px]" style={{ color: "var(--muted)" }}>
          Versioned vendor pricing. Historical cost uses the card in effect on that day.
        </p>
      </div>

      <div className="panel p-3">
        <h3 className="mb-2 text-sm font-medium">Cards</h3>
        <DataTable
          columns={[
            { key: "name", label: "Name" },
      ...
      {anthCards.length >= 2 && (
        <div className="panel p-3">
          <h3 className="mb-2 text-sm font-medium">Anthropic price cut — diff</h3>
                ...
                .map((b) => {
                  ...
                  return {
                    ...
                    delta: (
                      <span
                        style={{
                          color: aPrice < bPrice ? "var(--success)" : "var(--danger)",
                        }}
                      >
      ...
      <div className="panel p-3">
        <h3 className="mb-2 text-sm font-medium">All lines</h3>
        <DataTable
          columns={[
